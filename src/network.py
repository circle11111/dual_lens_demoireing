...
class my_model(nn.Module):
    ...
    def forward(self, x1, x2):
        y_1, y_2, y_3 = self.encoder(x1,x2)
        # print(y_1.size())
        # print(y_2.size())
        # print(y_3.size())
        # exit()
...
class Encoder(nn.Module):
    def __init__(self, feature_num, inter_num, sam_number):
        ...
        self.conv_combine = nn.Sequential(
            nn.Conv2d(feature_num*2, feature_num, kernel_size=3, stride=1, padding=1, bias=True),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x1, x2):
        ...
        # print("x2:")
        # print(x2)
        # x1 = F.pixel_unshuffle(x1, 2)
        # print("x1:")
        # print(x1.size())
        x1 = self.conv_first(x1)
        # x2 = F.pixel_unshuffle(x2, 2)
...
class Decoder_Level(nn.Module):
    def __init__(self, feature_num, inter_num, sam_number):
        ...
        for _ in range(sam_number):
            sam_block = SAM(in_channel=feature_num, d_list=(1, 2, 3, 2, 1), inter_num=inter_num)
            self.sam_blocks.append(sam_block)
        self.conv = conv(in_channel=feature_num, out_channel=3, kernel_size=3, padding=1)
